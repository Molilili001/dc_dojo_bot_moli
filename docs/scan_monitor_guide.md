...
| `/扫描监听提醒 配置` | 服务器配置面板（全服规则） | 管理员 |
| `/扫描监听提醒 帖子配置` | 帖子配置面板 | 帖主/管理员 |
| `/扫描监听提醒 频道配置` | 频道与分类配置面板 | 管理员 |
...
除了全服规则和帖子规则，管理员还可以为特定**频道**或**分类**设置规则：

- **频道规则**：针对特定文字频道或论坛频道设置的规则，对该频道及其内所有帖子生效
- **分类规则**：针对频道分类设置的规则，对该分类下所有频道及其帖子生效
...
1. 点击 **添加频道规则** 按钮
2. 从下拉菜单选择目标频道（文字频道或论坛频道）
3. 填写规则配置表单：

| 字段 | 说明 | 示例 |
|------|------|------|
| 触发词 | 多个用逗号分隔 | `你好, hello` |
| 匹配模式 | 精确/前缀/包含/正则 | `精确` |
| 动作类型 | 回复/回顶/反应/回复并反应 | `回复` |
| 回复内容 | 纯文本或JSON格式embed | `欢迎！` |
| 删除延迟 | 秒数，0表示不删除 | `300` |
...
**场景1：论坛频道专属规则**
- 为 "技术讨论" 论坛频道设置规则，当用户发送 "求助" 时自动回复帮助指南

**场景2：分类统一规则**
- 为 "社区交流" 分类设置规则，该分类下所有频道都会响应 "版规" 关键词
...
- **限制数量**：每个帖子最多 10 条规则
- **只在本帖生效**：帖子规则只影响当前帖子
- **优先级最高**：帖子规则优先于频道、分类和全服规则

---

## 规则优先级

系统采用 **4 层规则优先级**机制，从高到低依次为：
...
| 优先级 | 规则类型 | 生效范围 | 设置者 |
|--------|----------|----------|--------|
| 1️⃣ 最高 | 帖子规则 | 仅当前帖子 | 帖主/管理员 |
| 2️⃣ 高 | 频道规则 | 指定频道及其帖子 | 管理员 |
| 3️⃣ 中 | 分类规则 | 分类下所有频道 | 管理员 |
| 4️⃣ 低 | 全服规则 | 全服所有频道 | 管理员 |

### 匹配逻辑

当用户发送消息时：
1. 首先检查是否匹配**帖子规则**（如果在帖子内）
2. 如果没有匹配，检查**频道规则**
3. 如果没有匹配，检查**分类规则**
4. 如果没有匹配，检查**全服规则**
5. 一旦找到匹配的规则，立即执行，不再检查更低优先级的规则

### 示例

假设有以下规则：
- 全服规则：触发词 "帮助" → 回复通用帮助信息
- 分类规则（游戏分类）：触发词 "帮助" → 回复游戏相关帮助
- 频道规则（公告频道）：触发词 "帮助" → 回复如何阅读公告
- 帖子规则（某攻略帖）：触发词 "帮助" → 回复攻略目录

用户发送 "帮助" 时：
- 在攻略帖内 → 触发帖子规则，回复攻略目录
- 在公告频道 → 触发频道规则，回复阅读公告方法
- 在游戏分类其他频道 → 触发分类规则，回复游戏帮助
- 在其他地方 → 触发全服规则，回复通用帮助
