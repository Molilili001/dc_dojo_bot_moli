...
*   **交互式徽章墙**:
    *   新增独立的 `/我的徽章墙` 指令，允许用户以交互式卡片的形式，浏览所有已获得的、由管理员自定义的图片徽章。
 ...
 *   **`badge_image_url`**: (可选) 在JSON文件的顶层添加此字段。
 *   **值**: 必须是一个公开的、可直接访问的图片链接（URL直链）。
 *   **格式**: URL应以 `.png`, `.jpg`, `.jpeg`, `.gif`, 或 `.webp` 结尾。
 *   **效果**: 当用户通关此道馆后，他们就可以在 `/我的徽章墙` 指令中看到这个徽章。如果此字段被省略或为空，则该道馆没有徽章。
