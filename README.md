...
*   **究极道馆挑战与排行榜**:
    *   **终极考验**: 一种特殊的挑战模式，题库由服务器内 **所有** 已启用的道馆题目合并而成。
    *   **随机与计时**: 系统将从总题库中随机抽取50%的题目，用户需要在零错误的情况下，用时尽可能短地完成挑战。
    *   **实时排行榜**: 用户的最佳成绩将被记录在服务器的“究极道馆排行榜”上，并实时更新。
    *   **个人排名查询**: 在排行榜面板上，用户可以点击“查询我的排名”按钮，私密地查看自己的当前排名和最佳用时。
...
*   **失败冷却机制**: 多次挑战失败会触发1至12小时不等的冷却时间，防止恶意尝试（注：究极道馆无此限制）。
*   **智能错题回顾**:
...
*   **面板管理**:
    ...
    *   `/道馆 召唤排行榜`: 在指定频道创建一个可自动更新的**究极道馆排行榜**。支持自定义排行榜的标题和描述。
    *   `/道馆 徽章墙面板`: 在指定频道创建一个永久的**徽章墙展示面板**。
    *   `/道馆 毕业面板`: 在指定频道创建一个永久的**毕业面板**，用于发放“全部通关”奖励。
*   **用户管理**:
    *   `/道馆 重置进度`: 完全清空指定用户的所有挑战记录、失败记录、已领取的身份组奖励记录以及**究极道馆排行榜成绩**。
    *   `/道馆 解除处罚`: 立即移除用户的挑战冷却时间。
*   **通用工具**:
    *   `/say`: (管理员/开发者) 让机器人发送一条自定义消息。
        *   `message`: 要发送的文字内容。
        *   `channel` (可选): 指定消息发送的频道，默认为当前频道。
        *   `image` (可选): 附加一张图片。
*   **双层处罚系统**:
    *   **黑名单 (Blacklist)**:
...
*   **自动化联动**:
    *   **频道监听模式**: 可配置监听一个受信任的机器人伙伴在特定频道发送的消息。
        *   `{"punish": "用户ID"}`: 收到此格式的JSON消息后，会自动将该用户加入**黑名单**，并**完全重置其所有进度**。
*   **数据安全 (混合备份系统)**:
...
*   **开发者工具**:
    *   `/状态`: [仅限开发者] 查看VPS的实时CPU、内存、磁盘占用率以及机器人的运行时间和内存消耗，并支持下载当日日志。
...
1.  在项目根目录 (`F:\dcbot`) 下，找到 `config.json` 文件。
    ...
    **包含自动化联动功能的完整配置示例:**
    ```json
    {
      ...
      "AUTO_BLACKLIST_MONITOR": {
        ...
        "monitor_channel_id": "1401035278533136386"
      }
    }
    ```
...
这是一个高级功能，允许本机器人监听另一个受信任的机器人伙伴在**特定频道**发出的消息，并自动执行相应操作。

### 工作模式: 频道 JSON 监听

*   **工作原理**: 机器人会监听来自 `config.json` 中指定的 `target_bot_id` 在 `monitor_channel_id` 频道中发送的消息。当收到一条内容为特定JSON格式的消息时，它会立即触发相应操作。
*   **服务器内同步**: 所有通过此模式触发的操作都只在**当前服务器**内执行。
...
*   **处罚用户 (加入黑名单并重置进度)**
    *   **格式**: `{ "punish": "被处罚用户的ID" }`
    *   **效果**:
        1.  将指定用户加入当前服务器的**黑名单** (`cheating_blacklist`)。
        2.  **完全重置**该用户在当前服务器的道馆进度、失败记录和奖励记录。
...
*   `monitor_channel_id`: (字符串) 用于监听的频道的ID。
...
*   `randomize_options`: (布尔值, `true` 或 `false`) 是否随机打乱选择题的选项顺序。
